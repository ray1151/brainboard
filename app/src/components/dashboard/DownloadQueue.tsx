...
export function DownloadQueue({ items, onClearFinished, onCancelAll }: DownloadQueueProps) {
    ...
    return (
        <div className="fixed bottom-4 right-4 w-80 bg-brand-surface border border-brand-border rounded-xl shadow-2xl overflow-hidden z-[100]">
            <div className="p-3 border-b border-brand-border bg-brand-hover flex justify-between items-center">
                <div className="flex items-center gap-2">
                    <Download className="w-4 h-4 text-brand-secondary" />
                    <h4 className="text-sm font-medium text-brand-text">Downloads</h4>
                    {activeCount > 0 && (
                        <span className="text-xs px-1.5 py-0.5 bg-brand-secondary/20 text-brand-secondary rounded-full">
                            {activeCount} active
                        </span>
                    )}
                </div>
                <div className="flex gap-2">
                    ...
                    {completedCount > 0 && (
                        <button onClick={onClearFinished} className="text-xs text-brand-primary hover:text-brand-text transition-colors">
                            Clear Finished
                        </button>
                    )}
                </div>
            ...
            <div className="max-h-60 overflow-y-auto p-2 space-y-2">
                {items.map(item => (
                    <div key={item.id} className="flex flex-col gap-1 p-2 bg-brand-hover rounded">
                        <div className="flex items-center gap-3 text-sm">
                            <div className="flex-shrink-0">
                                ...
                                {item.status === 'downloading' && <div className="w-4 h-4 rounded-full border-2 border-brand-secondary border-t-transparent animate-spin" />}
                            ...
                            <div className="flex-1 truncate text-brand-subtext" title={item.filename}>
                                {item.filename}
                            </div>
                            {item.status === 'downloading' && item.progress !== undefined && (
                                <div className="text-xs text-brand-secondary font-mono">{item.progress}%</div>
                            )}
                            {item.status === 'cancelled' && <div className="text-xs text-gray-400">Cancelled</div>}
                        </div>
                        {item.status === 'downloading' && (
                            <div className="w-full bg-brand-border h-1 mt-1 rounded-full overflow-hidden">
                                {item.progress !== undefined ? (
                                    ...
                                    <div className="bg-brand-secondary h-full w-full animate-progress-indeterminate" />
                                )}
                            </div>
                        )}
