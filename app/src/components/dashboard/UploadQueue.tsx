    ...
    return (
        <div className="fixed bottom-4 right-4 w-80 bg-brand-surface border border-brand-border rounded-xl shadow-2xl overflow-hidden z-[100]">
            <div className="p-3 border-b border-brand-border bg-brand-hover flex justify-between items-center">
                <h4 className="text-sm font-medium text-brand-text">Uploads</h4>
                <div className="flex gap-2">
                    {hasPendingOrActive && (
                        <button onClick={onCancelAll} className="text-xs text-red-400 hover:text-red-300 transition-colors">Cancel All</button>
                    )}
                    <button onClick={onClearFinished} className="text-xs text-brand-primary hover:text-brand-text transition-colors">Clear Finished</button>
                </div>
            </div>
                ...
                {items.map(item => (
                    <div key={item.id} className="flex flex-col gap-1 p-2 bg-brand-hover rounded">
                        <div className="flex items-center gap-3 text-sm">
                            ...
                            <div className="flex-1 truncate text-brand-subtext" title={item.path}>
                                {item.path.split('/').pop()}
                            </div>
